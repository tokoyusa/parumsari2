...
export default function AdminPanel({ currentProfile, appSettings, onRefreshSettings }: AdminPanelProps) {
  ...
  const [adminSubTab, setAdminSubTab] = useState<'management' | 'settings' | 'tegalsari'>('management');
    ...
    <div className="bg-white rounded-2xl border border-emerald-100 shadow-sm overflow-hidden p-6 space-y-8">
      ...
      {adminSubTab === 'settings' && (
        ...
        <div className="pt-4">
          ...
          >
            {loading ? 'Menyimpan...' : adminSubTab === 'tegalsari' ? 'Simpan Konten Profil Desa Tegalsari' : 'Simpan Semua Setelan Global'}
          </button>
        </div>
      </form>
    </div>
